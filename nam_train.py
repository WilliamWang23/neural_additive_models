...
# Enable TF1 graph-mode semantics when running on TensorFlow 2.x.
...
def _evaluate_with_best_checkpoint(x_train, y_train,
                                   x_test, y_test,
                                   best_checkpoint_dirs):
  """Evaluates each model from its best checkpoint on a held-out test split."""
  test_scores = []
  ...
  for n in range(FLAGS.n_models):
    ...
    with tf.Graph().as_default():
      ...
      graph_tensors_and_ops, metric_scores = graph_builder.build_graph(
          ...
          name_scope=f'model_{n}')
      saver = tf.train.Saver(
          var_list=graph_tensors_and_ops['nn_model'].trainable_variables)
      checkpoint_path = tf.train.latest_checkpoint(best_checkpoint_dirs[n])
      if checkpoint_path is None:
        # Fallback for directories containing copied model.* files without a
        # checkpoint state file.
        index_files = gfile.glob(
            os.path.join(best_checkpoint_dirs[n], 'model*.index'))
        if index_files:
          checkpoint_path = sorted(index_files)[-1].replace('.index', '')
      if checkpoint_path is None:
      ...
      with tf.Session() as sess:
        ...
        test_scores.append(metric_scores['test'](sess))
...
def training(x_train, y_train, x_validation,
  ...
  Returns:
    ...
    If `return_test_metric` is True, additionally returns the test metric
    computed from best checkpoints.
  """
  ...
  mean_train_metric = np.mean(best_train_metric)
  mean_validation_metric = np.mean(best_validation_metric)
  if return_test_metric:
    if x_test is None or y_test is None:
      raise ValueError(
          'x_test and y_test must be provided when return_test_metric=True')
    mean_test_metric = _evaluate_with_best_checkpoint(
        x_train, y_train, x_test, y_test, best_checkpoint_dirs)
    return mean_train_metric, mean_validation_metric, mean_test_metric
...
def single_split_training(data_gen,
  ...
  fold_dir = os.path.join(logdir, 'fold_{}'.format(FLAGS.fold_num))
  gfile.makedirs(fold_dir)
  params_path = os.path.join(fold_dir, 'training_params.json')
  params = {
      'dataset_name': FLAGS.dataset_name,
      'regression': bool(FLAGS.regression),
      'cross_val': bool(FLAGS.cross_val),
      'activation': FLAGS.activation,
      'shallow': bool(FLAGS.shallow),
      'batch_size': int(FLAGS.batch_size),
      'training_epochs': int(FLAGS.training_epochs),
      'early_stopping_epochs': int(FLAGS.early_stopping_epochs),
      'decay_rate': float(FLAGS.decay_rate),
      'learning_rate': float(FLAGS.learning_rate),
      'output_regularization': float(FLAGS.output_regularization),
      'l2_regularization': float(FLAGS.l2_regularization),
      'dropout': float(FLAGS.dropout),
      'feature_dropout': float(FLAGS.feature_dropout),
      'n_models': int(FLAGS.n_models),
      'num_splits': int(FLAGS.num_splits),
      'fold_num': int(FLAGS.fold_num),
      'num_basis_functions': int(FLAGS.num_basis_functions),
      'units_multiplier': int(FLAGS.units_multiplier),
      'data_split': int(FLAGS.data_split),
  }
  with gfile.GFile(params_path, 'w') as f:
